// === AUTO_INJECTED_FPGA_ATTRS ===
(* use_dsp         = "no"          *)
(* use_carry_chain = "no"          *)
(* shreg_extract   = "no"          *)
// === END AUTO_INJECTED_FPGA_ATTRS ===
module mult_mod_3329_bits(A,B,R

    );

    input [12:1] A;
    input [12:1] B;
    output [12:1] R;
wire [6:1] r1;
wire [9:1] r2;
wire [12:1] r3;
wire [12:1] r4;
wire [9:1] r5;
wire [12:1] r6;
wire [12:1] r7;
wire [12:1] r8;
wire [12:1] r9;
wire [12:1] r10;
wire [12:1] r11;
wire [12:1] r12;
wire [12:1] r13;
wire [12:1] r14;
wire [12:1] r15;
wire [12:1] r16;
wire [15:1] temp_R_1;
//wire [6:1] r17;
//wire [9:1] r18;
//wire [12:1] r19;
wire [12:1] r20;
wire [13:1] temp_R_2;
reg [12:1] temp_R;

wire [10:1] temp_R_1_1;
wire [13:1] temp_R_1_2,temp_R_1_3,temp_R_1_4,temp_R_1_5;
wire [14:1] temp_R_1_6,temp_R_1_7;


mult_3x3_st  label1 (.x0(A[3]),.x1(A[2]),.x2(A[1]),
          .x3(B[3]),.x4(B[2]),.x5(B[1]),
          .z0(r1[6]),.z1(r1[5]),.z2(r1[4]),.z3(r1[3]),.z4(r1[2]),.z5(r1[1]));

mult_3x3_2_3_st  label2 (.x0(A[3]),.x1(A[2]),.x2(A[1]),
          .x3(B[6]),.x4(B[5]),.x5(B[4]),
          .z0(r2[9]),.z1(r2[8]),.z2(r2[7]),.z3(r2[6]),.z4(r2[5]),.z5(r2[4]),.z6(r2[3]),.z7(r2[2]),.z8(r2[1]));

mult_3x3_2_6_st  label3 (.x0(A[3]),.x1(A[2]),.x2(A[1]),
          .x3(B[9]),.x4(B[8]),.x5(B[7]),
          .z00(r3[12]),.z01(r3[11]),.z02(r3[10]),.z03(r3[9]),.z04(r3[8]),.z05(r3[7]),.z06(r3[6]),.z07(r3[5]),.z08(r3[4]),.z09(r3[3]),.z10(r3[2]),.z11(r3[1]));

mult_3x3_2_9_strong  label4 (.x0(A[3]),.x1(A[2]),.x2(A[1]),
          .x3(B[12]),.x4(B[11]),.x5(B[10]),
          .z00(r4[12]),.z01(r4[11]),.z02(r4[10]),.z03(r4[9]),.z04(r4[8]),.z05(r4[7]),.z06(r4[6]),.z07(r4[5]),.z08(r4[4]),.z09(r4[3]),.z10(r4[2]),.z11(r4[1]));


mult_3x3_2_3_st  label5 (.x0(A[6]),.x1(A[5]),.x2(A[4]),
          .x3(B[3]),.x4(B[2]),.x5(B[1]),
          .z0(r5[9]),.z1(r5[8]),.z2(r5[7]),.z3(r5[6]),.z4(r5[5]),.z5(r5[4]),.z6(r5[3]),.z7(r5[2]),.z8(r5[1]));

mult_3x3_2_6_st  label6 (.x0(A[6]),.x1(A[5]),.x2(A[4]),
          .x3(B[6]),.x4(B[5]),.x5(B[4]),
          .z00(r6[12]),.z01(r6[11]),.z02(r6[10]),.z03(r6[9]),.z04(r6[8]),.z05(r6[7]),.z06(r6[6]),.z07(r6[5]),.z08(r6[4]),.z09(r6[3]),.z10(r6[2]),.z11(r6[1]));

mult_3x3_2_9_strong  label7 (.x0(A[6]),.x1(A[5]),.x2(A[4]),
          .x3(B[9]),.x4(B[8]),.x5(B[7]),
          .z00(r7[12]),.z01(r7[11]),.z02(r7[10]),.z03(r7[9]),.z04(r7[8]),.z05(r7[7]),.z06(r7[6]),.z07(r7[5]),.z08(r7[4]),.z09(r7[3]),.z10(r7[2]),.z11(r7[1]));

mult_3x3_2_12_strong  label8 (.x0(A[6]),.x1(A[5]),.x2(A[4]),
          .x3(B[12]),.x4(B[11]),.x5(B[10]),
          .z00(r8[12]),.z01(r8[11]),.z02(r8[10]),.z03(r8[9]),.z04(r8[8]),.z05(r8[7]),.z06(r8[6]),.z07(r8[5]),.z08(r8[4]),.z09(r8[3]),.z10(r8[2]),.z11(r8[1]));


mult_3x3_2_6_st  label9 (.x0(A[9]),.x1(A[8]),.x2(A[7]),
          .x3(B[3]),.x4(B[2]),.x5(B[1]),
          .z00(r9[12]),.z01(r9[11]),.z02(r9[10]),.z03(r9[9]),.z04(r9[8]),.z05(r9[7]),.z06(r9[6]),.z07(r9[5]),.z08(r9[4]),.z09(r9[3]),.z10(r9[2]),.z11(r9[1]));

mult_3x3_2_9_strong  label10 (.x0(A[9]),.x1(A[8]),.x2(A[7]),
          .x3(B[6]),.x4(B[5]),.x5(B[4]),
          .z00(r10[12]),.z01(r10[11]),.z02(r10[10]),.z03(r10[9]),.z04(r10[8]),.z05(r10[7]),.z06(r10[6]),.z07(r10[5]),.z08(r10[4]),.z09(r10[3]),.z10(r10[2]),.z11(r10[1]));

mult_3x3_2_12_strong  label11 (.x0(A[9]),.x1(A[8]),.x2(A[7]),
          .x3(B[9]),.x4(B[8]),.x5(B[7]),
          .z00(r11[12]),.z01(r11[11]),.z02(r11[10]),.z03(r11[9]),.z04(r11[8]),.z05(r11[7]),.z06(r11[6]),.z07(r11[5]),.z08(r11[4]),.z09(r11[3]),.z10(r11[2]),.z11(r11[1]));

mult_3x3_2_15_strong  label12 (.x0(A[9]),.x1(A[8]),.x2(A[7]),
          .x3(B[12]),.x4(B[11]),.x5(B[10]),
          .z00(r12[12]),.z01(r12[11]),.z02(r12[10]),.z03(r12[9]),.z04(r12[8]),.z05(r12[7]),.z06(r12[6]),.z07(r12[5]),.z08(r12[4]),.z09(r12[3]),.z10(r12[2]),.z11(r12[1]));


mult_3x3_2_9_strong  label13 (.x0(A[12]),.x1(A[11]),.x2(A[10]),
          .x3(B[3]),.x4(B[2]),.x5(B[1]),
          .z00(r13[12]),.z01(r13[11]),.z02(r13[10]),.z03(r13[9]),.z04(r13[8]),.z05(r13[7]),.z06(r13[6]),.z07(r13[5]),.z08(r13[4]),.z09(r13[3]),.z10(r13[2]),.z11(r13[1]));

mult_3x3_2_12_strong  label14 (.x0(A[12]),.x1(A[11]),.x2(A[10]),
          .x3(B[6]),.x4(B[5]),.x5(B[4]),
          .z00(r14[12]),.z01(r14[11]),.z02(r14[10]),.z03(r14[9]),.z04(r14[8]),.z05(r14[7]),.z06(r14[6]),.z07(r14[5]),.z08(r14[4]),.z09(r14[3]),.z10(r14[2]),.z11(r14[1]));

mult_3x3_2_15_strong  label15 (.x0(A[12]),.x1(A[11]),.x2(A[10]),
          .x3(B[9]),.x4(B[8]),.x5(B[7]),
          .z00(r15[12]),.z01(r15[11]),.z02(r15[10]),.z03(r15[9]),.z04(r15[8]),.z05(r15[7]),.z06(r15[6]),.z07(r15[5]),.z08(r15[4]),.z09(r15[3]),.z10(r15[2]),.z11(r15[1]));

mult_3x3_2_18_strong  label16 (.x0(A[12]),.x1(A[11]),.x2(A[10]),
          .x3(B[12]),.x4(B[11]),.x5(B[10]),
          .z00(r16[12]),.z01(r16[11]),.z02(r16[10]),.z03(r16[9]),.z04(r16[8]),.z05(r16[7]),.z06(r16[6]),.z07(r16[5]),.z08(r16[4]),.z09(r16[3]),.z10(r16[2]),.z11(r16[1]));


//assign temp_R_1 = r1 + r2 + r3 + r4 + r5 + r6 + r7 + r8 + r9 + r10 + r11 + r12 + r13 + r14 + r15 + r16;

assign temp_R_1_1 = r1 + r2 + r5;

assign temp_R_1_2 = r3 + r6 + r7;

assign temp_R_1_3 = r8 + r9 + r10;

assign temp_R_1_4 = r11 + r12 + r13;

assign temp_R_1_5 = r14 + r15 + r16;


assign temp_R_1_6 = temp_R_1_1 + temp_R_1_2 + r4;
assign temp_R_1_7 = temp_R_1_3 + temp_R_1_4 + temp_R_1_5;

assign temp_R_1 = temp_R_1_6 + temp_R_1_7;


//mult_3_2_3_strong label17 (.x0(temp_R_1[6]),.x1(temp_R_1[5]),.x2(temp_R_1[4]),
//          .z0(r17[6]),.z1(r17[5]),.z2(r17[4]),.z3(r17[3]),.z4(r17[2]),.z5(r17[1]));

//mult_3_2_6_strong  label18 (.x0(temp_R_1[9]),.x1(temp_R_1[8]),.x2(temp_R_1[7]),
//	  .z0(r18[9]),.z1(r18[8]),.z2(r18[7]),.z3(r18[6]),.z4(r18[5]),.z5(r18[4]),.z6(r18[3]),.z7(r18[2]),.z8(r18[1]));

//mult_3_2_9_strong  label19 (.x0(temp_R_1[12]),.x1(temp_R_1[11]),.x2(temp_R_1[10]),
//	  .z00(r19[12]),.z01(r19[11]),.z02(r19[10]),.z03(r19[9]),.z04(r19[8]),.z05(r19[7]),.z06(r19[6]),.z07(r19[5]),.z08(r19[4]),.z09(r19[3]),.z10(r19[2]),.z11(r19[1]));

mult_3_2_12_strong  label20 (.x0(temp_R_1[15]),.x1(temp_R_1[14]),.x2(temp_R_1[13]),
          .z00(r20[12]),.z01(r20[11]),.z02(r20[10]),.z03(r20[9]),.z04(r20[8]),.z05(r20[7]),.z06(r20[6]),.z07(r20[5]),.z08(r20[4]),.z09(r20[3]),.z10(r20[2]),.z11(r20[1]));

//assign temp_R_2 = temp_R_1[3:1] + r17 + r18 + r19 + r20;

//wire [9:1] temp_R_2_1;
//wire [13:1] temp_R_2_2;


//assign temp_R_2_1 = temp_R_1[3:1] + r17 + r18;
//assign temp_R_2_2 = r19 + r20;

//assign temp_R_2 = temp_R_2_1  + temp_R_2_2;

assign temp_R_2 = temp_R_1[12:1]  + r20;


always @(temp_R_2)
begin
  if (temp_R_2 >= 3329)
    temp_R <= temp_R_2 - 3329;
  else
    temp_R <= temp_R_2;
end

assign R = temp_R;

endmodule